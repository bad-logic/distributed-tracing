...
func initialDatabaseTableSetup(){
	...
	_ , err := Db.Exec("CREATE TABLE IF NOT EXISTS product(Id INT NOT NULL AUTO_INCREMENT,Name VARCHAR(128) NOT NULL,ShortDesc VARCHAR(128),Price FLOAT NOT NULL,CreatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,UpdatedAt TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP ,UserId INT NOT NULL,PRIMARY KEY (`Id`))");
